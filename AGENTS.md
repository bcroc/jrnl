...
- Parser contract: an entry starts at a line matching `^## \d{4}-\d{2}-\d{2} \d{2}:\d{2}$`. Any body text beginning with `## ` + that shape is misparsed as a new entry; keep the block format of `saveEntry` and `parseEntries` in sync (both live in `src/jrnl.ts`).
